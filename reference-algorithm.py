...
def make_polygon():
    # only generate valid polygons with area > 0
    # Area of triangle  = 0.5 * (x1 * (y2-y3) + x2 * (y3 - y1) + x3 * (y1-y2))
    # formula is derived from shoelace formula (Gauss's area formula)
    area = 0
    while area == 0:
    ...
    return ([(random.randrange(0, 256), random.randrange(0, 256),
              random.randrange(0, 256), random.randrange(30, 61)), (x1, y1), (x2, y2), (x3, y3)])
...
def run():
    ...
    toolbox.register("individual", tools.initRepeat, creator.Individual, make_polygon, n=number_of_polygons)
    ...
    while g < number_of_generations and round(max(fits), 5) < 0.95:
        ...
        for child1, child2 in zip(offspring[::2], offspring[1::2]):
            ...
            if (child1.fitness.values[0]!=child2.fitness.values[0] and
                (child1.fitness.values[0]<mean_before_cx and
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    graph_title = 'Reference algorithm ' + target_image
